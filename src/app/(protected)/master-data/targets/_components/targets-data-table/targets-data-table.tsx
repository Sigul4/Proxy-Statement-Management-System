...
export function TargetsDataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
	...
	function handleRowClicked(row: Row<TData>) {
		const originalItem = row.original as TargetItem
		const id = originalItem.id

		router.push(`/master-data/targets/${id}`)
	}
