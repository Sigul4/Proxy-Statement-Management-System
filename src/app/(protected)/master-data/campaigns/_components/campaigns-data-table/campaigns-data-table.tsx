...
export function CampaignsDataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
	...
	function handleRowClicked(row: Row<TData>) {
		const originalItem = row.original as CampaignsItem
		const id = originalItem.id

		router.push(`/master-data/campaigns/${id}`)
	}
